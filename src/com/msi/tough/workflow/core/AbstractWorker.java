...
/**
 * @author jgardner
 * @param <T> the type of request handled by this worker
 * @param <V> the type of values returned by this worker
 */
public abstract class AbstractWorker<T extends Message, V extends Message> {
    private final static Logger logger = Appctx
            .getLogger(AbstractWorker.class.getName());
     ...
     * @throws Exception
     */
    protected V doWork(T req, Session session)
            throws Exception {
        String requestId = null;
        requestId = (String) ProtobufUtil.getRequiredField(req, "requestId");
        assert(requestId != null);
        try {
            return doWork(req, session, requestId);
        }
     ...
     * @throws Exception
     */
    protected V doWork(T req, Session session, String requestId)
            throws Exception {
        ...
        try
        {
            V result = doWork0(req, context);
            result = ProtobufUtil.setRequiredField(result, "requestId", requestId);
            result = ProtobufUtil.setRequiredField(result, "callerAccessKey", accessKey);
     ...
     * @param req
     */
    protected abstract V doWork0(T req, ServiceRequestContext context)
            throws Exception;
     ...
     * @param e
     */
    protected void mark(V ret, Exception e)
    {
