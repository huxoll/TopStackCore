...
public class AdminServlet extends HttpServlet {
	...
	public AdminServlet() {
		this(new HealthCheckServlet(), new MetricsServlet(), new PingServlet(),
				new ThreadDumpServlet(), DEFAULT_HEALTHCHECK_URI,
				DEFAULT_METRICS_URI, DEFAULT_PING_URI, DEFAULT_THREADS_URI);
	}

	public AdminServlet(HealthCheckServlet healthCheckServlet,
			MetricsServlet metricsServlet, PingServlet pingServlet,
			ThreadDumpServlet threadDumpServlet, String healthcheckUri,
			String metricsUri, String pingUri, String threadsUri) {
	...
	public void init(ServletConfig config) throws ServletException {
		...
		metricsUri = getParam(config.getInitParameter("metrics-uri"),
				metricsUri);
		pingUri = getParam(config.getInitParameter("ping-uri"), pingUri);
		threadsUri = getParam(config.getInitParameter("threads-uri"),
				threadsUri);
		healthcheckUri = getParam(config.getInitParameter("healthcheck-uri"),
				healthcheckUri);
	}
}
