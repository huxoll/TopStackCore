...
public class MetricsServlet extends HttpServlet implements
	...
	/**
	 * The attribute name of the {@link JsonFactory} instance in the servlet
	 * context.
	 */
	public static final String JSON_FACTORY_ATTRIBUTE = JsonFactory.class
			.getCanonicalName();
	...
	public MetricsServlet() {
		this(Clock.defaultClock(), VirtualMachineMetrics.getInstance(),
				MetricsUtil.getMetricsRegistry(), DEFAULT_JSON_FACTORY, true);
	}
	...
	public MetricsServlet(boolean showJvmMetrics) {
		this(Clock.defaultClock(), VirtualMachineMetrics.getInstance(),
				MetricsUtil.getMetricsRegistry(), DEFAULT_JSON_FACTORY,
				showJvmMetrics);
	}
		...
		json.writeStartObject();
		{
			...
			if (!bufferPoolStats.isEmpty()) {
				...
				json.writeStartObject();
				{
					...
					json.writeStartObject();
					{
						json.writeNumberField("count",
								bufferPoolStats.get("direct").getCount());
						json.writeNumberField("memoryUsed", bufferPoolStats
								.get("direct").getMemoryUsed());
						json.writeNumberField("totalCapacity", bufferPoolStats
								.get("direct").getTotalCapacity());
					}
					json.writeEndObject();
					...
					{
						json.writeNumberField("count",
								bufferPoolStats.get("mapped").getCount());
						json.writeNumberField("memoryUsed", bufferPoolStats
								.get("mapped").getMemoryUsed());
						json.writeNumberField("totalCapacity", bufferPoolStats
								.get("mapped").getTotalCapacity());
					}
					json.writeEndObject();
				}
				json.writeEndObject();
